...
def test_tool_module_structure():
    ...
    from taskgenie.tools import device_tools, app_tools, screen_tools
...
def test_server_py_syntax():
    ...
    try:
        ...
        with open("src/taskgenie/server.py", "r", encoding="utf-8") as f:
            server_code = f.read()

        # This will raise SyntaxError if there are syntax issues
        compile(server_code, "src/taskgenie/server.py", "exec")

    except SyntaxError as e:
        pytest.fail(f"server.py has syntax error: {e}")
    except FileNotFoundError:
        pytest.fail("src/taskgenie/server.py not found")
    except Exception as e:
...
def test_tools_modules_syntax():
    ...
    tool_modules = [
        "src/taskgenie/tools/__init__.py",
        "src/taskgenie/tools/device_tools.py",
        "src/taskgenie/tools/app_tools.py",
        "src/taskgenie/tools/screen_tools.py",
        "src/taskgenie/tools/input_tools.py",
        "src/taskgenie/tools/inspection_tools.py",
        "src/taskgenie/tools/advanced_tools.py",
    ]
